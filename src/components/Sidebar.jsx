...
import { UploadIcon, FileIcon, Spinner, EditIcon, SearchIcon, SettingsIcon, HelpIcon, TrashIcon } from "./icons";
...
function CollapsedSidebar({ user, onLogin, onToggleSidebar, onNewConversation }) {
  return (
  ...
  return (
    <aside className="w-72 bg-white border-r border-gray-200 flex flex-col shrink-0 h-screen shadow-sm">
      ...
      <div className="flex-1 overflow-y-auto px-3 py-2 space-y-4">
        ...
        {/* Upload section */}
        <div>
          <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2 px-1">Document</p>
          <div
          ...
          {uploadError && <p className="text-xs text-red-500 mt-1.5 px-1">{uploadError}</p>}
        ...
        {/* Chat History */}
        <div>
          <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2 px-1">History</p>
          {filtered.length === 0 ? (
      ...
      <div className="border-t border-gray-100 px-3 py-2 space-y-0.5">
        <button className="w-full flex items-center gap-2.5 px-3 py-2 rounded-xl text-sm text-gray-600 hover:bg-gray-100 hover:text-gray-900 transition-colors cursor-pointer">
          <SettingsIcon />
          Settings
        </button>
        <button className="w-full flex items-center gap-2.5 px-3 py-2 rounded-xl text-sm text-gray-600 hover:bg-gray-100 hover:text-gray-900 transition-colors cursor-pointer">
        ...
        {!user && (
          <div className="mt-2 rounded-xl bg-indigo-50 border border-indigo-100 p-3">
            <p className="text-xs font-semibold text-indigo-900 mb-1">Get responses tailored to you</p>
            <p className="text-[11px] text-indigo-700 mb-2.5">
              Log in to save chats, upload documents, and get personalized answers.
            </p>
            <button
              onClick={onLogin}
        ...
        {user && (
          <div className="relative mt-1">
            ...
            >
              ...
              <span className="text-sm text-gray-800 font-medium truncate flex-1 text-left">{user.name}</span>
            </button>
            {showUserMenu && (
