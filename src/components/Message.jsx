export function Message({ role, text, timestamp, sources, onSourceClick, activeSource }) {
  ...
  return (
    <div className={`flex gap-4 mb-6 ${isUser ? "justify-end" : "justify-start"}`}>
      ...
      {!isUser && (
        <div className="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center text-gray-700 text-xs font-bold shrink-0 mt-0.5">
          AI
        </div>
      )}
      ...
      <div className={`flex flex-col gap-1 ${isUser ? "items-end max-w-[75%]" : "items-start flex-1"}`}>
        ...
        {isUser ? (
          ...
          <div className="bg-white border border-gray-200 text-gray-900 px-4 py-3 rounded-2xl text-sm leading-relaxed whitespace-pre-wrap shadow-sm">
            {text}
          </div>
        )}

        <span className={`text-[11px] px-1 ${isUser ? "text-indigo-400" : "text-gray-400"}`}>
          {timestamp}
        </span>
        ...
        {!isUser && sources && sources.length > 0 && (
          <div className="mt-1 flex flex-wrap gap-1.5">
            ...
            <span className="text-[10px] font-semibold text-gray-500 uppercase tracking-wide self-center">
              Sources:
            </span>
